...
struct chunk_frame : public frame<int> {
    ...
    std::vector<char> get_data() {
        ...
        // body 1 + 4 + 5*4 = 25
        auto x_v = get_bytes_int(row_); //int result row
        std::memcpy(&(d[5]), x_v.data(), sizeof(unsigned int));

        auto y_v = get_bytes_int(col_); //int result col
        std::memcpy(&(d[9]), y_v.data(), sizeof(unsigned int));

        auto la_v = get_bytes_int(height1); //int height of 1st matrix
        std::memcpy(&(d[13]), la_v.data(), sizeof(unsigned int));

        auto lb_v = get_bytes_int(width2); //int width of 2nd matrix
        std::memcpy(&(d[17]), lb_v.data(), sizeof(unsigned int));

        auto n_v = get_bytes_int(l_); //int common length
